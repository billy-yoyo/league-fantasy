...
def parse_participant(game, data):
  # parse player 
  in_game_name_with_tag = data["riotIdGameName"]
  in_game_name = in_game_name_with_tag.strip().split(" ", 1)[1]
  ...
  return data["participantId"], player, player_stats
...
def scrape_match(game):
  ...
  for participant_data in data["participants"]:
    participant_id, player, player_stats = parse_participant(game, participant_data)
